...
const renderSingleLineBoxV5 = ({ editorInfo, depth, inDepthBlockIndex, lineBlockType, isfirstFromTopToDown, isFirstFromBottomToUp, lineZero, boxHeight, boxLeftEdge, boxRightEdge, optimalLeftOfRangePx, optimalRightOfRangePx, legitFirstLineZero, legitLastLineZero, isFocusedBlock, firstLineHasVisibleChar, lastLineHasVisibleChar, firstVisibleChar, lastVisibleChar, inputBorderColor, inputBackgroundColor, borderSize, currMaxDepthIndex, specialLineForCodelens, }) => {
    ...
    */
    ...
    if (isFocusedBlock && extension_1.glo.hideBlockOfFocusedSingleLineArea) {
        return;
    }
    ...
    if (isFocusedBlock) {
        borderStyle = extension_1.glo.borderStyleOfFocusedBlock;
    }
    else {
        borderStyle = extension_1.glo.borderStyleOfNonFocusedBlock;
    }
    ...
    if (isExpandableToRightAsMain) {
        if (eE.rightSideBaseOfBlocks === "innerC") {
            ...
            finalWidthCalcOfMain = `calc(350vw - ((${boxLeftEdge} * (1ch + ${extension_1.glo.letterSpacing}px)) + ${leftInc - borderSize}px))`;
        }
    ...
    const lineDecoration = vscode.window.createTextEditorDecorationType({
        before: {
            ...
            textDecoration: `;box-sizing: content-box !important;
                ...
                `calc((100% * ${extension_1.glo.ratioOfCodeLensHeightByEditorLineHeight}) + 2px)`};
                position: absolute;
                z-index: ${zIndex};
        ...
        if (width > 0 || isExpandableToRightAsClHelp) {
            ...
            if (isExpandableToRightAsClHelp) {
                if (eE.rightSideBaseOfBlocks === "innerC") {
                    ...
                    finalWidthCalcOfClHelp = `calc(350vw - ((${boxLeftEdge} * (1ch + ${extension_1.glo.letterSpacing}px)) + ${leftInc - borderSize}px))`;
                }
                else if (eE.rightSideBaseOfBlocks === "fileC") {
    ...
    editorInfo.editorRef.setDecorations(lineDecoration, !specialLineForCodelens ? arrayOfCurrRanges : [singleRange]);
};
