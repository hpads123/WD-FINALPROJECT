...
const applyAllBlockmanSettings = () => {
    ...
    const hideBlockOfFocusedSingleLineArea = bc.get("n27B01HideBlockOfFocusedSingleLineArea");
    if (hideBlockOfFocusedSingleLineArea) {
        extension_1.glo.hideBlockOfFocusedSingleLineArea = true;
    }
    else if (hideBlockOfFocusedSingleLineArea === false) {
        extension_1.glo.hideBlockOfFocusedSingleLineArea = false;
    }
    // ==============
    ...
    if (typeof renderIncrementBeforeAndAfterVisibleRange === "number" &&
        ...
        vscode.window.showInformationMessage(`The increment number should be between -200 and 200.`, { modal: false });
        // .then((x) => {
        //     console.log("x--->", x);
        // });
    }
    ...
    if (typeof customBlackListOfFileFormats === "string") {
        ...
        if (stringWithoutSpacesAndTabs) {
            ...
            const possibleWhiteListIndicator = stringWithoutSpacesAndTabs.slice(0, possibleWhiteListIndicatorLength);
            if (possibleWhiteListIndicator === "^") {
                // now it is a white list, not black list.
                const theFormatListString = stringWithoutSpacesAndTabs.slice(possibleWhiteListIndicatorLength);
    ...
    // =======================
    const newBorderThicknessOfNonFocusedBlock = bc.get("n34A01BorderThicknessOfNonFocusedBlock");
    const newBorderThicknessOfFocusedBlock = bc.get("n34A02BorderThicknessOfFocusedBlock");
    if (typeof newBorderThicknessOfNonFocusedBlock === "number") {
        extension_1.glo.borderThicknessOfNonFocusedBlock =
            newBorderThicknessOfNonFocusedBlock;
    }
    if (typeof newBorderThicknessOfFocusedBlock === "number") {
        extension_1.glo.borderThicknessOfFocusedBlock = newBorderThicknessOfFocusedBlock;
    }
    //
    const borderStyleOfFocusedBlock = bc.get("n34A03BorderStyleOfFocusedBlock");
    if (typeof borderStyleOfFocusedBlock === "string") {
        extension_1.glo.borderStyleOfFocusedBlock = borderStyleOfFocusedBlock;
    }
    const borderStyleOfNonFocusedBlock = bc.get("n34A04BorderStyleOfNonFocusedBlock");
    if (typeof borderStyleOfNonFocusedBlock === "string") {
        extension_1.glo.borderStyleOfNonFocusedBlock = borderStyleOfNonFocusedBlock;
    }
    // =======================
    const nowDisableRecommendationDialog = bc.get("n35A01DisableRecommendationDialog");
    if (nowDisableRecommendationDialog) {
        extension_1.glo.disableRecommendationDialog = true;
    }
    else if (nowDisableRecommendationDialog === false) {
        extension_1.glo.disableRecommendationDialog = false;
    }
    const nowMaxCharCountInAnyLine = bc.get("n36A01MaxCharCountInAnyLine");
    if (typeof nowMaxCharCountInAnyLine === "number") {
        extension_1.glo.maxCharCountInAnyLine = nowMaxCharCountInAnyLine;
    }
    // ==========================
    // -----
